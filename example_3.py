...
while True:
    ...
    if event == "-GRAPH-":  # Mouse clicks (mouse down) on Graph Elememt 
        x, y = values["-GRAPH-"]
        if not dragging: # On mouse down (which selects the shape)
            ...
            end_point = (x, y)
        if prior_rect:
            # Clears prior drawing event, particularly since mouse up hasn't been done yet
        ...
        if start_point and end_point: # If movement has been initiated on Graph, but not finished
            if values['-MOVE-']:
